...
def get_ensembl_info(ensembl_list):
    ...
    for i, ensembl_id in enumerate(ensembl_list):
        try:
            ...
            xref_response = requests.get(xref_url, headers={"Content-Type": "application/json"})
            xref_response.raise_for_status()
            xref_data = xref_response.json()

            uniprot_id = next((x['primary_id'] for x in xref_data if x['dbname'] == 'UniProt/SWISSPROT'), 'Not found')
    ...
    for i, gene in enumerate(gene_list):
        ...
        try:
            ...
            if data['results']:
                ...
                recommended_name = entry['proteinDescription']['recommendedName']['fullName']['value'] if 'proteinDescription' in entry and 'recommendedName' in entry['proteinDescription'] else 'Not found'
                
                function = next((comment['texts'][0]['value'] for comment in entry.get('comments', []) 
                                 if comment['commentType'] == 'FUNCTION'), 'Not found')
                
                subcellular_location = next((loc['location']['value'] for comment in entry.get('comments', [])
                                             if comment['commentType'] == 'SUBCELLULAR LOCATION' 
                                             for loc in comment.get('subcellularLocations', [])), 'Not found')
                
                structural_family = next((comment['texts'][0]['value'] for comment in entry.get('comments', [])
                                          if comment['commentType'] == 'SIMILARITY'), 'Not found')
...
def main():
    ...
    st.markdown("<h1 style='text-align: center; color: green;'>Ensembl Gene Information Retriever</h1>", unsafe_allow_html=True)

    # Input for gene IDs or symbols
    input_text = st.text_area("Enter Ensembl IDs or HGNC symbols (one per line):", 
                              "ENSG00000234186\nENSG00000184385\nIL10\nHDC\nENSG00000180251\nFAM216B\nDRD3\nENSG00000229590")
        ...
        if input_list:
            ...
            if hgnc_symbols:
                with st.spinner('Retrieving information for HGNC symbols...'):
                    ...
                    st.success(f'Retrieved information for {len(hgnc_symbols)} HGNC symbols')
