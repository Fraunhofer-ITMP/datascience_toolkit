...
def saveFiles(kgName, disease2protein, drugAdvEffect, final_kg, drug_df,snp_df,uprot_dict):
    ...
    st.write('Please enter the location (e.g. \'C:\\Users\\rkarki\\Documents\\kg\\\' ) where KG files should be stored. A folder will be created automatically.','\n')
...
def GetDiseaseSNPs(disease_id): 
    ...
    try:
        ...
        st.write('No SNPs found. This could be either because 1. The identifier of your disease of interest is not compatible with Experimental Factor Ontology (EFO) or 2. No SNPs have been reported for the disease')
...
def snp2gene_rel(snp_df,graph): 
    ...
    st.write('A total of ' + str(len(snp_df)) + ' SNPs have been identified from GWAS Central. Now adding relevant data')
    st.write('\n')
...
def createKG():
    ...
    if not drugs_df.empty:
        ...
        st.write('A total of ' + str(len(list(set(drugs_df['drugId'])))) + ' drugs have been identified. Now fetching relevant data')
...
def GetSmiles(drugs_df,colname):
    ...
    for item in tqdm(drugs_list,desc='Getting SMILES for CHEMBL ids and generating descriptors'):
        ...
        try:
            ...
            st.write('This ChEMBL id could not be parsed: ' + str(item))
            #st.write(item)
            continue
    
    st.write('Reason: Drug has no SMILES representation because its type is either antibody, protein or unknown')
...
#     Heavy atom count between 15 and 50
        ...
        #st.write(molecular_weight,logp,h_bond_donor,h_bond_acceptors,formal_charge,rotatable_bonds,heavy_atoms)
...
def druglikeness_parallelCoordinates(df):
    ...
    st.write('This plot excludes drugs with molecular weight > 2000')
