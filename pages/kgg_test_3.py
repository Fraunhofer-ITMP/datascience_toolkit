...
def getQuery():
    ...
    st.markdown("**:blue[Welcome to the KG Generator tool. In the following steps, we will need some inputs from your side.]**")
...
def disease_df(query):
    ...
    if query:
        ...
        if not search_df.empty:
            st.write('**:blue[Here you go! Hopefully your disease of interest is in the list. If so, let\'s get started.]**')
...
def get_inputs():
    ...
    with st.form(key='get_disease'):
        ...
        efo_id = st.text_input('Please enter the index value of your disease of interest.',value=0)
    ...
    if st.session_state.efo_id is not None and st.session_state.ctphase is not None and st.session_state.kg_name is not None:
        st.success('All 3 inputs saved', icon="✅")
        st.session_state.query_phaseCompleted = True
        return st.session_state.efo_id, st.session_state.ctphase, st.session_state.kg_name
...
def GetDiseaseAssociatedProteinsPlot(df):
    ...
    st.header(
        "Summary of proteins related to the disease(s)",
        divider="gray"
    )
...
def snp2gene_rel(snp_df, graph):
    ...
    st.write(
        'A total of ' + str(len(snp_df)) + ' SNPs have been identified from GWAS Central. Now adding relevant data')
    st.write('\n')
...
def getViralProteins(query_disease):
    ...
    if not virus_subset_1.empty:
        st.write('\n')
        st.write('The workflow has identified your query as a viral disease. Its proteins (SWISS-Prot) will be now represented in the KG.', '\n')
        st.write('Enter Viral Prot Function')

        with st.form(key='identify_viral_proteins'):
            virus_name = st.text_input("Do you want to look further for a specific virus? Please type its name or skip it by typing 'no': ",
                                       value=st.session_state.virus_name)
            submit_button = st.form_submit_button(label='Search',on_click=virus_form_callback())
        ...
        if st.session_state.virus_form_submitted:
            ...
            if st.session_state.virus_name.lower() != 'no':
                st.session_state.virus_subset_2 = virus[
                    virus['virus name'].str.contains(st.session_state.virus_name, na=False, case=False)]
...
if st.session_state.kg_finalized:
    ...
    if st.button('Start New KG'):
        for key in ['query_generated', 'query', 'search_df', 'query_phaseCompleted', 'efo_id', 'ctphase', 'kg_name',
                    'initial_kg_created', 'kg_finalized', 'drugs_df', 'dis2prot_df', 'dis2snp_df']:
            if key in st.session_state:
                del st.session_state[key]
        st.rerun()
